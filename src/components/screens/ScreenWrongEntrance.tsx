...
const ScreenWrongEntrance: React.FC<{ onNavigate: (id: string) => void }> = ({ onNavigate }) => (
  <div className="space-y-4">
    <div className="flex items-center gap-3 px-1">
      <div className="w-12 h-12 rounded-full bg-[hsl(28,95%,52%,0.14)] flex items-center justify-center flex-shrink-0">
        <AlertTriangle className="w-6 h-6 text-[hsl(28,95%,52%)]" />
      </div>
      <div>
        <h2 className="transit-hero-text">Wrong entrance</h2>
        <p className="text-sm text-muted-foreground">You may be in the Residence Wing</p>
      </div>
    </div>

    <div className="transit-card">
      <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Signs you're in the wrong area</p>
    ...
    <div className="transit-card">
      <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">How to recover</p>
